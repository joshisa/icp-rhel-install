...
# Move RSA key to va node
for ((i=0; i < $NUM_VA; i++)); do
  # Prevent SSH identity prompts
  # If host IP exists in known hosts remove it
  ssh-keygen -R ${VA_IPS[i]}
  # Add host IP to known hosts
  ssh-keyscan -H ${VA_IPS[i]} | tee -a ~/.ssh/known_hosts

  # Copy over key (Will prompt for password)
  scp ~/.ssh/id_rsa.pub ${SSH_USER}@${VA_IPS[i]}:~/id_rsa.pub
  ssh ${SSH_USER}@${VA_IPS[i]} 'mkdir -p ~/.ssh; cat ~/id_rsa.pub | tee -a ~/.ssh/authorized_keys'
done
