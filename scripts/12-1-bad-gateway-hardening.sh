...
kubectl patch statefulset/icp-ds -n kube-system -p '{"spec":{"template":{"spec":{"containers":[{"env":[{"readinessProbe":{"failureThreshold":3,"periodSeconds":2}}]}]}}}}'
...
echo "Your statefulset failureThreshold setting is now updated to $(kubectl get statefulset icp-ds -n kube-system -o jsonpath='{.spec.template.spec.containers[*].readinessProbe.failureThreshold}')"
echo ""
...
echo "Your statefulset periodSeconds setting is now update to $(kubectl get statefulset icp-ds -n kube-system -o jsonpath='{.spec.template.spec.containers[*].readinessProbe.periodSeconds}')"
...
if [ "$STRATEGY" -eq "RollingUpdate" ]; then
  ...
  kubectl set image ds/calico-node-amd64 calico-node-amd64=calico/node:v2.6.7 install-cni=calico/cni:v1.11.2
  echo "    Updating calico-policy-controller deployment image version ..."
  kubectl set image deploy/calico-policy-controller calico-policy-controller=calico/kube-controllers:v1.0.3
