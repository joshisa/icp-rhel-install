...
for ((i=0; i < $NUM_WORKERS; i++)); do
    ssh ${SSH_USER}@${WORKER_HOSTNAMES[i]} sudo shutdown -r --now
done

echo "Looping through proxies ..."
for ((i=0; i < $NUM_PROXIES; i++)); do
    ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo shutdown -r --now
done

echo "Looping through management ..."
for ((i=0; i < $NUM_MANAGERS; i++)); do
    ssh ${SSH_USER}@${MANAGEMENT_HOSTNAMES[i]} sudo shutdown -r --now
done
